...
import com.ggod.utils.MyCookieUtil;
...
public class ProductServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		/**
		 * 1.获取请求参数 <br>
		 * 2.获取cookie数组，通过指定的名称（自己指定）查找cookie <br>
		 * 3.如果cookie==null，第一次访问 如果是第一次访问，创建cookie，回写浏览器 <br>
		 * 4.如果cookie!=null，不是第一次访问 <br>
		 * 如果不是第一次访问，说明我的cookie中已经存在 id 判断，当前的id是否已经存在cookie的中value 如果存在，不用操作
		 * 如果不存在，在后面追加（product=1,2）<br>
		 * 5.重定向到商品页面
		 */
		...
		// 重定向到商品页面
		response.sendRedirect("/jspcookiedemo/cookie/productList.jsp");
	}
